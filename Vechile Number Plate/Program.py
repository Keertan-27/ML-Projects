...
images_dir = "F:\STUDY\Python Projects\1 Projects\Vechile Number Plate\Images"

for img in glob.glob(images_dir + "/*.jpeg"):
    img = cv2.imread(img)
...
for img_path in glob.glob(current_dir + "/search/*.jpeg"):
...
if search_results:
    print("The car numbers to search are:", search_results)

    for res2 in search_results:
        result = binarySearch(array, 0, len(array) - 1, res2)
        if result != -1:
            print(f"\nThe Vehicle with number {res2} is allowed to visit.")
        else:
